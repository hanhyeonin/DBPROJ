...
public class OracleConnectionTest {
	...
	public static void main(String[] args) {
		...
		// DB접속 정보
		String url = "jdbc:oracle:thin:@192.168.1.36:1521:XE";   //데이터베이스 URL - 디비버에서 확인가능합니다.
						// jdbc:oracle:thin는 지정된 프로토콜		(@뒤에 ip로도 가능 192.168.1.36:1521:XE)
					//jdbc프로토콜,오라클 지정 ,localhost는 서버에 접속시 ip로 대체 , 1521은 오라클의 포트(서비스 구별값)
		String driver= "oracle.jdbc.driver.OracleDriver";     
		// ㄴ 오라클 드라이버클래스(패키지명.클래스명) - 외부라이브러리 ojdbc6.jar 필요
		String user ="iclass10";			//접속할 계정 정보 - 계정이름
		String password="1234";		// 			           ㄴ 패스워드
		
		try {				// 오류 처리
			// 1. 메모리에 데이터베이스 드라이버 클래스를 로드합니다.(로드-> 적재)
			Class.forName(driver);	// forName() 메소드는 throws ClassNotFoundException 처리
