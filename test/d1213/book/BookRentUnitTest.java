...
class BookRentUnitTest {
	...
	@Order(1)
	@DisplayName("대여 중인 도서 목록의 크기는 count 함수값과 통일해야한다.")
	@Test
...
//		fail("Not yet implemented");		// fail 메소드는 검증 실패시 실행
	}
	...
	@ParameterizedTest
	@ValueSource(strings = {"C1101","A1102"})
	// @Test
	...
	@ParameterizedTest
	@ValueSource(ints = {10002, 10003})
	void isAvailableMemberTest(int mem_idx) throws SQLException {
		boolean result = dao.isAvailableMember(mem_idx);
		assertTrue(result);
	}
	
	@Order(5)
	@DisplayName("회원 10004 의 대여 정보가 있으므로 - 기대값 not null")
	@Test
	...
	@Order(7)
	@DisplayName("회원 10004, 도서 C1101 대여 실행 - 기대값 1")
	@Test
	void rentTest2() {
		try {
			int cnt = dao.rentBook(10004, "C1101");
			assertEquals(1, cnt);
		} catch (SQLException e) {
			System.out.println("대여 오류 : 잘못된 회원 또는 도서 입니다.");
		}
...
/*		
 * 	JUnit : 단위테스트(사용자가 만든 메소드 중심)를 위한 도구(외부라이브러리 추가해야함)
 * 	단위테스트는 데이터가 준비(given)되면 어떤 메소드를 실행(when)시켰을때 결과가 
 * 	기대하는(then) 값이 나와야 하는것을 검증하는 과정입니다.
 * 	
